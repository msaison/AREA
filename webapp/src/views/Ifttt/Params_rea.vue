...
export default {
  ...
  async mounted() {
    this.url_data = sessionStorage.getItem("reaction_service");
    this.trig_type = sessionStorage.getItem("reaction_type");
    this.url_data = this.url_data == 'GOOGLE' ? 'GMAIL' : this.url_data
    axios
      ...
      .then((response) => {
        ...
        this.apiActions.forEach((element) => {
          if (element.type == this.url_data) {
            ...
            element.reactions.forEach((element_) => {
              if (element_.type == this.trig_type) {
                if (element_.params.length > 0) {
                  ...
                  sessionStorage.setItem("reaction_params", JSON.stringify([{ "": "" }]));
                  this.$router.push("/");
                }
              }
            });
  ...
  methods: {
    ...
    paramsTab() {
      ...
      sessionStorage.setItem("reaction_params", JSON.stringify(this.frontParams))
    },
    paramsConfirm() {
      this.$router.push("/");
    },
